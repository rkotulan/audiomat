...
def _engine_slug_for_cache(proj) -> str:
    """Canonical engine slug for the preview cache key. Mirrors the
    same fold rule the render-path manifest signature uses
    (:meth:`audiomat.render.ProjectRenderer._params_signature`): ``None``
    and the literal ``"default"`` both collapse to ``"default"`` so a
    project that toggles between the two doesn't double-invalidate.

    Critically, ``tts_model`` MUST participate in the preview cache key
    — otherwise switching a project's engine mid-flight leaves stale
    cells on disk that look "cached" to the UI but were generated by a
    different model. Discovered during v0.5.1 testing on Rezavý les v2
    when OmniVoice → Higgs swap kept serving OmniVoice samples.
    """
    return getattr(proj, "tts_model", None) or "default"
        ...
        for idx, v in enumerate(matrix):
            try:
                ...
                key_src = (
                    ...
                    f"|engine={_engine_slug_for_cache(proj)}"
                )
...
def preview_custom(slug: str, req: PreviewCustomRequest):
    ...
    key_src = (
        f"{clean}|{req.num_step}|{req.guidance_scale}|{req.speed}|{voice.name_slug}"
        f"|engine={_engine_slug_for_cache(proj)}"
    )
    key = hashlib.md5(key_src.encode("utf-8")).hexdigest()[:16]
